...
func GetUserBasicBy_Identity(Identity string) (*UserBasic, error) {
	ub := new(UserBasic)
	err := Mongo.Collection((&UserBasic{}).CollectionName()).FindOne(context.Background(), bson.D{{Key: "uid", Value: Identity}}).Decode(ub)
	return ub, err
}
